...
int vz_cpu_parse_cpuid_override(void)
{
	...
	while (fgets(s, sizeof(s), f)) {
		...
		if (sscanf(s, "%x %x: %x %x %x %x", &e.op, &e.count, &e.eax, &e.ebx, &e.ecx, &e.edx) == 6) {
			e.has_count = true;
		} else if (sscanf(s, "%x: %x %x %x %x", &e.op, &e.eax, &e.ebx, &e.ecx, &e.edx) == 5) {
			e.count = 0;
			e.has_count = false;
		} else {
		...
		new = realloc(vz_cpuid_override_entries, (nr_vz_cpuid_override_entries + 1) * sizeof(e));
		if (!new) {
...
static vz_cpuid_override_entry_t *vz_cpuid_override_lookup(unsigned int op, bool has_count, unsigned int count)
{
	size_t i;

	for (i = 0; i < nr_vz_cpuid_override_entries; i++) {
		if (vz_cpuid_override_entries[i].op != op || vz_cpuid_override_entries[i].has_count != has_count ||
...
static inline void vz_cpuid(unsigned int op, unsigned int *eax, unsigned int *ebx, unsigned int *ecx, unsigned int *edx)
{
	vz_cpuid_override_entry_t *e;
...
static inline void vz_cpuid_count(unsigned int op, int count, unsigned int *eax, unsigned int *ebx, unsigned int *ecx,
				  unsigned int *edx)
{
	vz_cpuid_override_entry_t *e;
...
int compel_cpuid(compel_cpuinfo_t *c)
{
	...
	/* Get vendor name */
	vz_cpuid(0x00000000, (unsigned int *)&c->cpuid_level, (unsigned int *)&c->x86_vendor_id[0],
