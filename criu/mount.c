...
static char *get_clean_mnt(struct mount_info *mi, char *mnt_path_tmp, char *mnt_path_root)
{
	...
	if (mount(mi->ns_mountpoint, mnt_path, NULL, MS_BIND, NULL)) {
		pr_warn("Can't bind-mount %d:%s to %s: %s\n",
			  mi->mnt_id, mi->ns_mountpoint, mnt_path, strerror(errno));
		rmdir(mnt_path);
		return NULL;
	}
 ...
 * return values:
 *  - ret >= 0, mount fd on successfull mount and open.
...
static __maybe_unused int mount_and_open_binfmt_misc(unsigned int *s_dev)
{
	...
	if (mount("binfmt_misc", target_dir, "binfmt_misc", 0, NULL)) {
		...
		pr_perror("Failed to mount binfmt_misc at %s",
			target_dir);
		return BINFMT_MOUNT_FAILED;
	}
...
static int __restore_shared_options(char *mountpoint, int mnt_id, bool private,
				    bool shared, bool slave)
{
	if (private && mount(NULL, mountpoint, NULL, MS_PRIVATE, NULL)) {
		pr_perror("Unable to make %d:%s private", mnt_id, mountpoint);
		return -1;
	}
	if (slave && mount(NULL, mountpoint, NULL, MS_SLAVE, NULL)) {
		pr_perror("Unable to make %d:%s slave", mnt_id, mountpoint);
		return -1;
	}
	if (shared && mount(NULL, mountpoint, NULL, MS_SHARED, NULL)) {
		pr_perror("Unable to make %d:%s shared", mnt_id, mountpoint);
		return -1;
	}
...
static int restore_shared_options(struct mount_info *mi, bool private, bool shared, bool slave)
{
	...
	return __restore_shared_options(service_mountpoint(mi), mi->mnt_id,
...
static int do_bind_mount(struct mount_info *mi)
{
	...
	if (__restore_shared_options(mnt_path, mi->mnt_id, private,
				     mi->shared_id && !shared,
				     mi->master_id && !master))
		goto err;
...
static int do_mount_one(struct mount_info *mi)
{
	...
	if ((mi->parent != root_yard_mp) &&
	    !strcmp(mi->parent->ns_mountpoint, mi->ns_mountpoint)) {
	...
	if (rst_mnt_is_root(mi)) {
		...
		if (root_ns_mask & CLONE_NEWUSER) {
			...
			if (mount(opts.root, service_mountpoint(mi), NULL, flags, NULL)) {
				pr_perror("Unable to mount %s %s (id=%d)", opts.root, service_mountpoint(mi), mi->mnt_id);
				return -1;
			}
		}
...
int read_mnt_ns_img(void)
{
	...
	if (use_mounts_v2() && read_mnt_ns_img_v2(mntinfo))
		return -1;
					   ...
					   enum tree_step *step)
{
	...
	if (!list_empty(&mi->children))
		return list_entry(mi->children.next, struct mount_info,
				  siblings);
	...
	while (mi->parent && mi != root) {
		if (mi->siblings.next == &mi->parent->children) {
			...
			return list_entry(mi->siblings.next, struct mount_info,
					  siblings);
		}
	}
...
struct mount_info *mnt_subtree_next(struct mount_info *mi,
				    struct mount_info *root)
{
