...
struct mount_info *lookup_mnt_sdev_on_root(unsigned int s_dev)
{
	...
	for (m = mntinfo; m != NULL; m = m->next) {
		if (kdev_to_odev(m->s_dev) == s_dev && is_root(m->root))
			return m;
	}
...
static struct mount_info *mount_resolve_path(struct mount_info *mntinfo_tree, const char *path)
{
	...
	BUG_ON(!__get_relative_path(path, mntinfo_tree->ns_mountpoint));

	while (1) {
		struct mount_info *c, *t = NULL;

		list_for_each_entry(c, &m->children, siblings) {
			if (!__get_relative_path(path, c->ns_mountpoint))
				continue;

			if (!t || get_relative_path(t->ns_mountpoint, c->ns_mountpoint))
				t = c;
		}
...
struct mount_info *lookup_first_fstype(int code)
{
	...
	for (m = mntinfo; m != NULL; m = m->next)
		if (m->fstype->code == code && is_root(m->root))
			return m;
	return NULL;
}
...
{
	if (bind->external && bind->rmi->mounted && is_sub_path(mi->root, bind->root))
		return true;
...
static bool unsupported_mount(const struct mount_info *m)
{
	...
	if (parent && nfs_mount(parent)) {
		...
		pr_err("\ttype %s source %s mnt_id %d s_dev %#x %s @ %s flags %#x options %s\n",
		       m->fsname, m->source, m->mnt_id, m->s_dev, m->root, m->ns_mountpoint, m->flags, m->options);
