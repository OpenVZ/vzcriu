...
int restore_socket_bufsz(int sk, SkOptsEntry *soe)
{
	...
	return userns_call(sk_setbufs, 0, bufs, sizeof(bufs), sk);
}
...
int fini_dump_sockets(struct ns_id *ns)
{
	...
	req.r.n.sdiag_protocol	= NDIAG_PROTO_ALL;
	req.r.n.ndiag_show	= NDIAG_SHOW_GROUPS | NDIAG_SHOW_FLAGS;
	tmp = do_collect_req(nl, &req, sizeof(req),
			     netlink_final_check_one, collect_err,
			     ns, (void *)(unsigned long)st.st_ino);
	if (tmp)
		err = tmp;
