...
static int prepare_pstree_for_shell_job(pid_t pid)
{
	...
	if (old_gid != current_gid && !lookup_create_item(current_gid, root_item->ids->pid_ns_id))
		return -1;
add_fake_session_leader:
	if (old_sid != current_sid && !lookup_create_item(current_sid, root_item->ids->pid_ns_id))
		return -1;
	return 0;
}
...
static int prepare_pstree_ids(pid_t pid)
{
	...
	list_for_each_entry(item, &root_item->children, sibling) {
		...
		if (leader->pid->state != TASK_UNDEF) {
			...
			if (helper_pid < 0)
				break;
			helper = lookup_create_item(helper_pid, item->ids->pid_ns_id);
			if (helper == NULL)
				return -1;
