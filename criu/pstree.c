...
static int prepare_pstree_for_shell_job(pid_t pid)
{
	...
	if (old_gid != current_gid) {
		...
		for_each_pstree_item(pi) {
			if (current_gid != current_sid && vpgid(pi) == current_gid) {
	...
	if (old_gid != current_gid && !lookup_create_item(&current_gid, 1, root_item->ids->pid_ns_id))
		return -1;
add_fake_session_leader:
	if (old_sid != current_sid && !lookup_create_item(&current_sid, 1, root_item->ids->pid_ns_id))
		return -1;
	return 0;
}

static struct pid *find_pid_or_place_in_hier(struct rb_node **root, pid_t pid, int level, struct rb_node **ret_parent,
					     struct rb_node ***ret_place)
{
...
again:
		...
		if (found) {
			if (found->item->replaceable) {
				...
				pr_info("%d: Removing old (short cut) pid on all lower levels\n", found->ns[0].virt);
				for (j = i; j >= 0; j--) {
		...
		pid_t pgid, pid_t *pitem_pid, pid_t *pnew_pgid)
{
	 ...
	 *  some synchronization. But we can still use them as a hint
...
static void prepare_pstree_my_child_subreaper(void)
{
	...
	for_each_pstree_item(item) {
		...
		if (parent->child_subreaper) {
			...
			pr_debug("Ancestor %d is my(%d) child subreaper copy from %d\n",
				 vpid(parent->my_child_subreaper), vpid(item), vpid(parent));
