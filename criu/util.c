...
static inline int get_proc_fd(int pid)
{
	if (pid == PROC_SELF) {
		...
		/**
		 * FIXME in case two processes from different pidnses share
		 * both files and pids - this check will not detect that we
		 * reuse /proc/self of a different process and everything proc
		 * related will break. Note that service_fd_id does not help
		 * here either.
		 */
...
{
	int status, ret = -1;
	bool old_in_child_process = in_child_process;
	pid_t pid;
