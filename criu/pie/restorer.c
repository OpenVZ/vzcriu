...
static int wait_zombies(struct task_restore_args *task_args)
{
	...
	for (i = 0; i < task_args->zombies_n; i++) {
		...
		if (ret == -ECHILD) {
			 ...
			 * won't be altered and we use nanosleep with for 100 ms
	...
	*args->breakpoint = rst_sigreturn;
	...
	if (args->nr_threads > 1) {
		...
		for (i = 0; i < args->nr_threads; i++) {
			...
			if (fd >= 0) {
				/* One level pid ns hierarhy */
				last_pid_len = std_vprint_num(last_pid_buf, sizeof(last_pid_buf),
							      thread_args[i].pid[0] - 1, &s);
				...
				for (k = 0; k < args->level; k++) {
					...
					if (request_set_next_pid(args->pid_ns_id[k], thread_args[i].pid[k],
								 args->transport_fd) < 0) {
