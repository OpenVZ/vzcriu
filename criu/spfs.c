...
static int create_reg_file(int ns_root_fd, const char *file_path, mode_t mode, size_t size)
{
	...
	if (size && ftruncate(fd, size)) {
		pr_perror("failed to truncate %s to %lu bytes", file_path, (unsigned long)size);
		err = -errno;
		goto close_fd;
	}
...
char *spfs_manager_socket_path(void)
{
	...
	if (strlen(socket_path) == 0) {
		int n = snprintf(socket_path, PATH_MAX, "%s/%s", spfs_manager_work_dir(), SPFS_MANAGER_SOCK_FILE);
		BUG_ON(n >= PATH_MAX);
	}
	return socket_path;
}
			      ...
			      unsigned long mountflags)
{
	...
	mount = xsprintf("mount;id=%d;mode=restore;mountpoint=%s;"
			 "ns_pid=%d;root=%s",
			 mi->mnt_id, mountpoint, root_item->pid->real, opts.root);
	if (!mount) {
		pr_err("failed to allocate mount request\n");
		goto free_freeze_cgroup;
	}

	replace = xsprintf("replace;id=%d;source=%s;type=%s;flags=%ld;bindmounts=%s;freeze_cgroup=%s;", mi->mnt_id,
			   source, type, mountflags, bindmounts, freeze_cgroup);
	if (!replace) {
...
int spfs_mount(struct mount_info *mi, const char *source, const char *filesystemtype, unsigned long mountflags)
{
	int ret;
	int ns_fd;
	int sock;
