...
{
	if (type == SOCK_DGRAM && (state == TCP_CLOSE || state == TCP_ESTABLISHED))
		return 1;
...
restore_queue:
	if (peer->queuer == ui && !(peer->ue->uflags & UNIX_UFLAGS__EXTERN) && restore_unix_queue(fd, peer))
		return -1;
restore_sk_common:
	...
	if (unlikely(ui->flags & USK_BINDMOUNT)) {
		...
		if (ret < 0) {
			pr_perror("Can't create %s id %#x",
				  pair ? "socketpair" : "socket", ui->ue->id);
			return -1;
		}
	}
...
static int open_unixsk_standalone(struct unix_sk_info *ui, int *new_fd)
{
	...
	if (ui->ue->uflags & UNIX_UFLAGS__SERVICE) {
		 ...
		 */
		if (ui->peer && (ui->peer->ue->uflags & UNIX_UFLAGS__EXTERN) && !(opts.ext_unix_sk)) {
...
int add_fake_unix_queuers(void)
{
	...
	list_for_each_entry(ui, &unix_sockets, list) {
		if ((ui->ue->uflags & (UNIX_UFLAGS__EXTERN | UNIX_UFLAGS__CALLBACK)) || ui->queuer)
			continue;
		if (!(ui->ue->type != SOCK_DGRAM && ui->ue->state == TCP_ESTABLISHED && !ui->peer) &&
