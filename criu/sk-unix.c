...
static int resolve_unix_ghosts_cb(struct pprep_head *ph);
static MAKE_PPREP_HEAD(resolve_unix_ghosts);
...
static int dump_one_unix_fd(int lfd, u32 id, const struct fd_parms *p)
{
	...
	*fown = p->fown;
	...
	/*
	 * @mnt_id is a sign for bindmounted socket,
	 * it must be predefined type/state and not
	 * deleted.
	 */
	if (sk->mnt_id) {
		ue->has_mnt_id	= true;
		ue->mnt_id	= sk->mnt_id;
		if (sk->deleted) {
			pr_err("Unix socket %#x is deleted, unsupported\n", ue->ino);
			goto err;
		}
	}
...
static int collect_one_unixsk(void *o, ProtobufCMessage *base, struct cr_img *i)
{
	...
	add_post_prepare_cb_once(&resolve_unix_ghosts);
