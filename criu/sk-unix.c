...
static int dump_one_unix_fd(int lfd, uint32_t id, const struct fd_parms *p)
{
	...
	*fown = p->fown;
	...
	sk = (struct unix_sk_desc *)lookup_socket(p->stat.st_ino, PF_UNIX, 0, true);
	...
	if (ue->peer) {
		peer = (struct unix_sk_desc *)lookup_socket(ue->peer, PF_UNIX, 0, true);
...
static int collect_one_unixsk(void *o, ProtobufCMessage *base, struct cr_img *i)
{
	 ...
	 */
	if (ui->ue->has_mnt_id && !ui->ue->has_ns_id) {
