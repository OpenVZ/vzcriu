...
TimensEntry *timens_ve_clock_fallback(void)
{
	...
	if (sscanf(opts.ve_clock_fallback,
		   "monotonic:%"PRId64".%"PRId64",boottime:%"PRId64".%"PRId64,
		   &te->monotonic->tv_sec, &te->monotonic->tv_nsec,
		   &te->boottime->tv_sec, &te->boottime->tv_nsec) != 4) {
