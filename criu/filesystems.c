...
static int dump_binfmt_misc_entry(int dfd, char *name, struct cr_img *img)
{
	...
	pr_debug("binfmt_misc_pattern=:%s:E::%s::%s:%s\n",
		 bme.name, bme.extension, bme.interpreter, bme.flags ?: "\0");
	ret = 0;
err:
...
static int sysfs_after_parse(struct mount_info *pm, bool for_dump)
{
	...
	if (nsfd < 0) {
		pr_warn("Failed to get netns tag of sysfs mount %d. Old kernel? %s\n",
...
static int fill_overlayfs_info(struct mount_info *mi)
{
	...
	for (i = 0; i < ofsi->nr_opts; i++) {
		...
		if (!strncmp(ofsi->opts[i], "lowerdir_mnt_id=", strlen("lowerdir_mnt_id="))) {
		...
		if (!strncmp(ofsi->opts[i], "upperdir_mnt_id=", strlen("upperdir_mnt_id="))) {
			   ...
			   char *fstype, unsigned long mountflags)
{
	...
	if (ofsi->upper) {
		...
		ofsi->options = xstrcat(ofsi->options, ",upperdir=%s,workdir=%s", upper_opt + 1, work_opt + 1);
		if (!ofsi->options) {
