...
static inline int fork_update_curr(struct pstree_item *item)
{
	if (!item->parent) {
		...
		real_sid = getsid(0);
		if (real_sid == -1) {
			pr_perror("%d: Fail to getsid", vpid(item));
			return -1;
		}

		real_pgid = getpgid(0);
		if (real_pgid == -1) {
			pr_perror("%d: Fail to getpgid", vpid(item));
			return -1;
		}
...
static int prepare_signals(int pid, struct task_restore_args *ta, CoreEntry *leader_core)
{
	...
	for (i = 0; i < current->nr_threads; i++) {
		if (!current->core[i]->thread_core->signals_p) /*backward compatibility*/
			ret = open_signal_image(CR_FD_PSIGNAL, vtid(current, i), &siginfo_priv_nr[i]);
		else
