...
static int open_core_VZ730(int pid, CoreEntry **pcore)
{
	...
	if (ret > 0) {
		...
		core->tc->has_old_seccomp_mode	= tc.has_seccomp_mode;
		core->tc->old_seccomp_mode	= tc.seccomp_mode;
		core->tc->has_old_seccomp_filter= tc.has_seccomp_filter;
		core->tc->old_seccomp_filter	= tc.seccomp_filter;
...
static void restore_pgid(void)
{
	...
	if (my_pgid != last_level_pid(current->pid)) {
		...
		if (leader) {
			BUG_ON(current->pgid->ns[0].virt != vpid(leader));
