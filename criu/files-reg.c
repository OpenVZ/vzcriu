...
static int open_remap_spfs(struct reg_file_info *rfi, RemapFilePathEntry *rfe)
{
	struct ghost_file *gf;
	GhostFileEntry *gfe = NULL;
	struct cr_img *img;
...
static int dump_ghost_file(int _fd, u32 id, const struct stat *st, dev_t phys_dev, bool dump_content)
{
	struct cr_img *img;
	int exit_code = -1;
...
static int dump_ghost_remap_type(char *path, const struct stat *st, int lfd, u32 id, struct ns_id *nsid,
...
static int create_link_remap(char *path, int len, int lfd, u32 *idp, struct mount_info *mi,
			     const struct fd_parms *parms, bool *fallback)
{
	...
	while (true) {
		...
		if (overmounted) {
			...
			path_root = mntns_root;
			rpath = link_name;
			fd = lfd;
		}

		ret = linkat_hard(fd, "", path_root, rpath, ost->st_uid, ost->st_gid, AT_EMPTY_PATH);
		if (overmounted) {
			close(path_root);
			close(fd);
		}
...
static int dump_linked_remap(char *path, int len, const struct fd_parms *parms, int lfd, u32 id, struct mount_info *mi,
			     bool *fallback)
{
	RemapFilePathEntry rpe = REMAP_FILE_PATH_ENTRY__INIT;
	...
	if (!find_link_remap(path, mi, &lid)) {
		...
		if (remap_type == REMAP_TYPE__SPFS_LINKED)
			return 0;
	} else if (create_link_remap(path, len, lfd, &lid, mi, parms, fallback))
		return -1;
...
static int dump_spfs_remap(char *path, const struct stat *st, int lfd, u32 id, struct ns_id *nsid)
{
			    ...
			    int lfd, u32 id, struct mount_info *mi, int is_overmounted)
{
	...
	if (ret < 0) {
		...
		if (errno == ENOENT) {
			link_strip_deleted(link);
			ret = dump_linked_remap(link->name + 1, plen - 1, parms, lfd, id, mi, &fallback);
			if (ret < 0 && fallback) {
...
static bool unsupported_nfs_file(const struct fd_link *link, const struct fd_parms *parms)
{
	if (parms->fs_type != NFS_SUPER_MAGIC)
		return false;
...
int dump_one_reg_file(int lfd, u32 id, const struct fd_parms *p)
{
	...
	if (!skip_for_shell_job && path_is_overmounted(link->name, mi)) {
		...
		pr_info("Dumping file %s on overmounted mount %d: will use relative path on restore\n",
			link->name, mi->mnt_id);
		if (check_mount_v2())
			pr_warn("Mounts-v2 is required to restore such image, but current kernel does not support it\n");
	}
	...
	if (!skip_for_shell_job && check_path_remap(link, p, lfd, id, mi, rfe.vz_use_relative_path))
		return -1;
...
out_root:
	*level = make_parent_dirs_if_need(path_root, path);
	...
	if (linkat_hard(remap_root, rpath, path_root, path, rfi->remap->uid, rfi->remap->gid, 0) < 0) {
		int errno_saved = errno;

		if (!rm_parent_dirs(path_root, path, *level) && errno_saved == EEXIST) {
			errno = errno_saved;
			ret = 1;
			goto err;
		}
...
ext:
		...
		if (rfi->rfe->has_mode) {
			...
			if (curr_mode != saved_mode) {
				...
				if (!strncmp(rfi->path, PROCFS_SYSDIR, strlen(PROCFS_SYSDIR)))
					...
					pr_warn("\tCgroup dirs might be modified outside. Ignore.\n");
				else
					goto err;
			}
		}
	...
	if (rfi->remap) {
		if (!rfi->remap->is_dir) {
			...
			if (mi && try_remount_writable(mi, REMOUNT_IN_REAL_MNTNS))
				goto err;
