	...
	while (file->path != NULL) {
		...
		if (file->dir) {
			...
			if (write(fd, file->content, len) != len) {
				pr_perror("Failed to write %s to file %s",
					  file->content, path);
				close(fd);
				return -1;
			}
...
static int mount_one(struct mountinfo *mi)
{
	...
	test_msg("Mounting %s %d %s %s %d\n", mi->mountpoint, mi->bind,
		 mi->fstype, mi->source, mi->mounted);
	...
	if (mi->bind != -1) {
		snprintf(source, sizeof(source), "%s/%s/%s", dirname,
			 mounts[mi->bind].mountpoint, mi->bind_root);
		fstype = NULL;
		mflags = MS_BIND;
	} else {
