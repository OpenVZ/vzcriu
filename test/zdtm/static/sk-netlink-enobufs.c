...
static int get_netlink_sk_drops(unsigned long sk_ino)
{
	...
	if (!netlink) {
		pr_perror("Failed to open /proc/net/netlink file");
		return -1;
	}
...
exit_close:
	if (fclose(netlink)) {
		pr_perror("Failed to close /proc/net/netlink file");
		return -1;
	}
...
int main(int argc, char **argv)
{
	 ...
	 */
	sk_bsize = 1;
	if (setsockopt(sk, SOL_SOCKET, SO_RCVBUF, &sk_bsize, sizeof(sk_bsize)) == -1) {
		pr_perror("Can't set rcv buf");
		close(sk);
		return 1;
	}

	if (getsockopt(sk, SOL_SOCKET, SO_RCVBUF, &sk_bsize, &optlen) == -1) {
