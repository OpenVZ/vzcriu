...
static int get_user_ns(pid_t pid, unsigned long *ns_id)
{
	...
	if (len < 0) {
		pr_perror("Can't read link %s", path);
		return -1;
	}

	buf[len] = '\0';
	/* ns_id can be unsigned long see nsfs_show_path */
	if (sscanf(buf, "user:[%lu", ns_id) != 1) {
...
static int write_map(pid_t pid, char *map)
{
	char path[PATH_MAX];
	int fd, ret;
	...
	ret = write(fd, "0 0 1\n", 6);
	if (ret != 6) {
		pr_perror("Can't write to %s", path);
		close(fd);
		return -1;
	}
	close(fd);
...
static int grand_child(pid_t pid1)
{
	char path[PATH_MAX];
	int fd = -1, ret;
...
static int child2(pid_t pid1)
{
	...
	*grand_child_pid = gc_pid;
	...
	if (status) {
		pr_err("Grand child exited with status %d\n", status);
		ret = 4;
		goto err;
	}
...
int main(int argc, char **argv)
{
	...
	unsigned long ns_p, ns_c1, ns_c2;
	int status;
	...
	if (get_user_ns(my_pid, &ns_p) < 0 ||
		...
		pr_perror("Can't get user ns");
		goto err;
	}

	if (ns_p == ns_c1 || ns_p != ns_c2) {
		fail("ns_p=%lu, ns_c1=%lu, ns_c2=%lu", ns_p, ns_c1, ns_c2);
		goto err;
	}
	...
	if (waitpid(pid1, &status, 0) < 0 || status) {
		fail("pid1: status=%d", status);
		goto err;
	}

	if (waitpid(pid2, &status, 0) < 0 || status) {
		fail("pid2: status=%d", status);
		goto err;
	}
	pass();
	return 0;
