...
/*
 * Create a child in new pid_ns; then the child creates thread and zombie.
...
static int read_ns_link(const char *file_name, ino_t *id)
{
	...
	sprintf(ns_path, "/proc/%d/task/%ld/ns/%s", getpid(), gettid(), file_name);
...
/*
 * To be able to use /proc/<getpid()>/task/<gettid()>/ns/<ns> we need to mount
 * the proc of the current pid namespace
 */
static int mount_current_proc(void)
{
	if (unshare(CLONE_NEWNS) < 0) {
		pr_perror("Can't unshare mntns");
		return -1;
	}

	if (mount(NULL, "/", NULL, MS_PRIVATE | MS_REC, NULL)) {
		pr_perror("Can't make-rprivate root");
		return -1;
	}
...
int main(int argc, char **argv)
{
	...
	if (waitpid(pid, &status, 0) != pid || !WIFEXITED(status) || WEXITSTATUS(status)) {
		fail("Can't wait or bad status: status=%d", status);
		return 1;
	}
