...
int inotify_setup(struct test_context *ctx)
{
	...
	ctx->watch_fd = inotify_add_watch(ctx->notify_fd, ctx->ovl_dir, IN_ALL_EVENTS);
...
#define IN_MASK_PRINT(a)                                                              \
	do {                                                                          \
		if (mask & IN_##a) {                                                  \
			n += snprintf(buf + n, bufsz - n, "%s" #a, first ? "" : " "); \
			first = 0;                                                    \
		}                                                                     \
	} while (0)
...
static int inotify_event_process(struct test_context *ctx, const struct inotify_event *i_event, struct action *a)
{
	char path[PATH_MAX];
	...
	if (!is_same_filepath(path, a->f->filepath)) {
		fail("inotify event filepath mismatch: expected: %s, got:%s", a->f->filepath, path);
		return -1;
	}

	if (a->event_mask != i_event->mask) {
		fail("inotify event mask mismatch: expected:%08x, got:%08x", a->event_mask, i_event->mask);
		return -1;
	}
...
int inotify_process_events(struct test_context *ctx, const char *events, int num_bytes, struct action **action,
			   struct action *end_action)
{
	int i;
