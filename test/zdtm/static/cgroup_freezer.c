...
{
	ssprintf(fz->dir, "%s/%s", path, name);
	if (mkdir(fz->dir, 0700) < 0) {
		pr_perror("Can't make freezer dir [%s]", fz->dir);
		return -1;
	}

	ssprintf(fz->state_path, "%s/freezer.state", fz->dir);
	ssprintf(fz->self_freezing_path, "%s/freezer.self_freezing", fz->dir);
	ssprintf(fz->tasks_path, "%s/tasks", fz->dir);
				   ...
				   enum freezer_index fz_index)
{
	...
	ssprintf(parent_path, "%s/%s", dirname, freezecg);
