...
function add_accept_rules {
	...
	${JOIN_CT} iptables-legacy -w -I ${CRTOOLS_IPTABLES_TABLE} -p tcp -s $server --sport $port -j ACCEPT &&
	${JOIN_CT} iptables-legacy -w -I ${CRTOOLS_IPTABLES_TABLE} -p tcp -d $server --dport $port -j ACCEPT &&
	${JOIN_CT} iptables-legacy -w -I ${CRTOOLS_IPTABLES_TABLE} -p udp -s $server --sport $port -j ACCEPT &&
	${JOIN_CT} iptables-legacy -w -I ${CRTOOLS_IPTABLES_TABLE} -p udp -d $server --dport $port -j ACCEPT
}
